...
const Tags = {
    ...
    task: {
        attrs: [
            ...
            { key: "activiti:externalFlowMaping", mapping: "workflow.externalFlowMaping" },
        ...
        genXml(context) {
            ...
            let assginees = approvals.filter(a => a.type == 'assginee');
            ...
            let assgineeStr = assginees.length ? ' activiti:candidateUsers="${processStartUser}"' : ''
            //根据属性配置,创建内容
            ...
            // }
            // 检查参数字段
            let userAttributesStr = '';
            if (context.workflow.userAttributes && context.workflow.userAttributes.length > 0) {
                const userAttributesJson = {};
                context.workflow.userAttributes.forEach(item => {
                    userAttributesJson[item.field] = item.value
                });
                userAttributesStr = ` activiti:userAttributes="${compile.entityUpload(JSON.stringify(userAttributesJson))}"`;
            }
            // 检查外部系统调用字段
            let externaParamsStr = '';
            if (context.workflow.externaParams && context.workflow.externaParams.length > 0) {
                const externaParamsJson = {};
                context.workflow.externaParams.forEach(item => {
                    externaParamsJson[item.field] = item.value
                });
                externaParamsStr = ` activiti:externaParams="${compile.entityUpload(JSON.stringify(externaParamsJson))}"`;
            }
            //构建标签并返回
            return `<${tagName} ${attrStr} ${userStr} ${groupStr} ${assgineeStr} ${userAttributesStr} ${externaParamsStr} > ${contents.join("")}</${tagName}>`
        },
        node2Meta(json, uidata) {
            let uiAttr = uidata.children[0].attributes
            let meta = {
            ...
            if (users) {
                if (users === '${processStartUser}') {
                    approvals.push({ type: "assginee", value: "" })
                } else {
            ...
            if (merges) {
                ...
                if (groups && groups[1])
                    approvals.push({ type: "group", value: groups[1].split(",") })
            ...
            // 外部系统参数
            if (json.attributes['activiti:externaParams']) {
                let externaParamsJson = JSON.parse(compile.entityLocal(json.attributes['activiti:externaParams']));
                meta.workflow.externaParams = Object.keys(externaParamsJson).map(item => {
                    return {
                        field: item,
                        value: externaParamsJson[item]
                    }
                })
            }
            // 参数
            if (json.attributes['activiti:userAttributes']) {
                let userAttributesJson = JSON.parse(compile.entityLocal(json.attributes['activiti:userAttributes']));
                meta.workflow.userAttributes = Object.keys(userAttributesJson).map(item => {
                    return {
                        field: item,
                        value: userAttributesJson[item]
                    }
                })
            }
            meta.workflow.showExternalFlowMaping = !(json.attributes['activiti:isStartTask'] === 'true')
