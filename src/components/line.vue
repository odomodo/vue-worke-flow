<template>
  <g v-if="edge" @mousedown="selectLine($event, edge)">
    <path
      :d="tempD"
      :marker-end="(isFinishSequence?'url(#mark-arrow-red)':'url(#mark-arrow)')"
      class="link"
      :class="{'selected': edge.selected, 'isFinishSequence': isFinishSequence}"
    ></path>

    <path v-if="!readonly&&!edge.selected" :d="tempD" class="link hide"></path>
    ...
    <g v-if="textInfo">
      <mytext :showTextBg="true" :x="textInfo.x" :y="textInfo.y" :desc="textInfo.desc"></mytext>
    </g>
...
export default {
  ...
  methods: {
    ...
    getTextInfo() {
      let textInfo = {}
      if (this.edge && this.edge.workflow && this.edge.workflow.hasCondition && (this.edge.workflow.condition || this.edge.workflow.condition === false) && this.lineList.length > 0) {
        let line = {}
        ...
        textInfo = {
          x: line.x1,
          y: line.y1,
          desc: this.edge.workflow.condition
        }
