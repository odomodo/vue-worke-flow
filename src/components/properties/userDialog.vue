...
export default {
  ...
  methods: {
    ...
    query() {
      ...
      // 0.管理员 1.平台 2.企业
      try {
        const accountType = JSON.parse(sessionStorage.getItem('vuex')).app.userInfo.accountType;
        this.realSearchOption.accountType = accountType;
      } catch (e) {
        console.log(`没找到：JSON.parse(sessionStorage.getItem('vuex')).app.userInfo.accountType`);
      }
      this.getList()
    },
