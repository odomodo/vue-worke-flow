  ...
  <div class="props-editor">
    <el-form size="small" label-width="100px" label-position="right">
      <template v-for="(field, index) in fields">
        <!-- 分割线 -->
        <el-divider v-if="field.type == 'divider' && checkField(field)" :key="index"
          ><span>{{ field.label }}</span></el-divider
        >
        <!-- text 普通文本框-->
        <el-form-item
          v-if="field.type == 'input' && checkField(field)"
          :label="field.label"
          :key="field.mapping"
        >
          <el-input
        ...
        >
          <!-- 选择子流程 -->
          <template v-if="isMdc">
            <mdcsubflow :nodeData="data" />
          </template>
        </el-form-item>

        <!-- externalFlowMaping -->
        <el-form-item
          v-if="field.type == 'externalFlowMaping' && checkField(field)"
          :label="field.label"
          :key="index"
        >
          <!-- 选择外部子流程 -->
          <template v-if="isMdc">
            <externalFlowMaping v-model="field.value" :nodeData="data" @change="changeValue(field, $event)" />
          </template>
        </el-form-item>
...
import mdcsubflow from "../mdc/subflowList.vue";
import externalFlowMaping from "../mdc/externalFlowMaping.vue";
...
export default {
  ...
  components: { PropField, mdcsubflow, externalFlowMaping },
  watch: {
    data(newval) {
