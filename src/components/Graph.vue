        ...
        >
          ...
          <g class="graph">
            ...
            <g class="edges-list" @click.stop>
              <myline v-for="edge in metadata.edges" :key="edge.id" :edge="edge" :nodes="metadata.nodes" :progress="progress" :readonly="readonly"></myline>
            </g>

            <g class="nodes-list">
              <item v-for="(node, index) in metadata.nodes" :key="index" :node="node" :dotLink="dotLink" :progress="progress" @startLinkNode="startLinkNode" @endLinkedNode="endLinkedNode" :readonly="readonly"></item>
            </g>

            <graticule v-if="!readonly" :node="graticuleNode" :metadata="metadata"></graticule>

            <virtualnode v-if="!readonly" :node="virtualNodeData"></virtualnode>

            <boxesselected v-if="!readonly" :boxesSelectedInfo="boxesSelectedInfo"></boxesselected>
...
export default {
  ...
  components: { definiens, myline, item, virtualnode, graticule, boxesselected },
  data() {
    return {
      // 是否正在拖曳整个框选
      isDragBox: false,
  ...
  created() {
    ...
    eventBus.$on(EVENTS.GRAPH_LINE_SELECT, (edge) => {
      if (edge.selected && this.boxesSelectedInfo && this.boxesSelectedInfo.start && this.boxesSelectedInfo.end) {
        this.isDragBox = true
      } else {
  ...
  computed: {
    styleWrap() {
      if (this.size && this.size < 1) {
        return `width: ${this.width * this.size}px; height: ${this.height * this.size}px; overflow: hidden;`
      } else {
        return ''
      }
    },
  ...
  methods: {
    ...
    handleKeyMove(e) {
      ...
      if (this.boxesSelectedInfo && this.boxesSelectedInfo.start && this.boxesSelectedInfo.end) {
        ...
        } else if (e.code === 'ArrowDown' && (this.boxesSelectedInfo.end.y + 1) <= this.height) {
          this.boxesSelectedInfo.start.y += 1
          this.boxesSelectedInfo.end.y += 1
        } else if (e.code === 'ArrowLeft' && this.boxesSelectedInfo.start.x >= 1) {
          this.boxesSelectedInfo.start.x -= 1
          this.boxesSelectedInfo.end.x -= 1
        } else if (e.code === 'ArrowRight' && (this.boxesSelectedInfo.end.x + 1) < this.width) {
          this.boxesSelectedInfo.start.x += 1
          this.boxesSelectedInfo.end.x += 1
        } else {
          return
      ...
      nodes.forEach(item => {
        if (e.code === 'ArrowUp' && item.y >= 1) {
        ...
        } else if (e.code === 'ArrowDown' && (item.linkNode.bottom.y + 1) <= this.height) {
          item.y += 1
          item.linkNode.top.y += 1
          item.linkNode.right.y += 1
          item.linkNode.bottom.y += 1
          ...
          moveNodes.push(item.id)
        } else if (e.code === 'ArrowRight' && (item.linkNode.right.x + 1) <= this.width) {
          item.x += 1
          item.linkNode.top.x += 1
          item.linkNode.right.x += 1
          item.linkNode.bottom.x += 1
      ...
      this.metadata.edges.forEach(edge => {
        if (edge.selected && moveNodes.includes(edge.source) && moveNodes.includes(edge.target)) {
        ...
        } else if (moveNodes.some(id => (edge.source === id || edge.target === id))) {
    ...
    handleDelete() {
      ...
      nodes.forEach(item => {
        this.metadata.edges.forEach(edge => {
          if ((edge.source === item.id || edge.target === item.id) && lines.indexOf(edge) === -1) {
            lines.push(edge)
          }
        })
    ...
    svgMousedown(e) {
      ...
      if (!this.selectedLine && !this.isLinking && !this.isDragNode && !this.isDragBox) {
    ...
    svgMousemove(event) {
      ...
      if (this.isDragLine) { // 处理节点间连线轨迹拖曳
        eventBus.$emit(EVENTS.GRAPH_LINE_MOUSEMOVE, event, this.isDragLine)
      } else if (this.isLinking && node !== null) { // 正在对节点进行连线
        this.lineDragData = this.caclPathDragData(node, event)
      } else if (this.isDragNode && node !== null) { // 正在拖曳节点
        if (this.virtualNodeData === null) {
          this.virtualNodeData = JSON.parse(JSON.stringify(node))
        }

        // 确定虚拟节点位置并处理节点过界
        this.virtualNodeData.x = Math.max(Math.floor(node.x + ((pageX - this.dragStart.x) / this.size)), 0);
        this.virtualNodeData.x = this.virtualNodeData.x + this.virtualNodeData.w > this.width ? Math.floor(this.width - this.virtualNodeData.w) : this.virtualNodeData.x
        this.virtualNodeData.y = Math.max(Math.floor(node.y + ((pageY - this.dragStart.y) / this.size)), 0);
        this.virtualNodeData.y = this.virtualNodeData.y + this.virtualNodeData.h > this.height ? Math.floor(this.height - this.virtualNodeData.h) : this.virtualNodeData.y
      ...
      } else if (this.tempNode) { // 新增节点的虚拟节点

        // 确定新增节点位置并处理节点过界
        let tempNodeX = Math.max(Math.floor(event.offsetX - this.tempNode.w / 2), 0);
        tempNodeX = tempNodeX + this.tempNode.w > this.width ? Math.floor(this.width - this.tempNode.w) : tempNodeX
        let tempNodeY = Math.max(Math.floor(event.offsetY - this.tempNode.h / 2), 0);
        tempNodeY = tempNodeY + this.tempNode.h > this.height ? Math.floor(this.height - this.tempNode.h) : tempNodeY
        ...
        this.graticuleNode = this.virtualNodeData
      } else if (this.isDragBox) { // 正在拖曳整个框选
        ...
        if (Math.abs(dragBoxX) > 0 || Math.abs(dragBoxY) > 0) {
          ...
          this.boxesSelectedInfo.start.x = Math.max(Math.floor(this.boxesSelectedInfo.box.x + dragBoxX), 0);
          this.boxesSelectedInfo.start.x = this.boxesSelectedInfo.start.x + this.boxesSelectedInfo.box.w > this.width ? Math.floor(this.width - this.boxesSelectedInfo.box.w) : this.boxesSelectedInfo.start.x
          dragBoxX = this.boxesSelectedInfo.start.x - oldStartX

          const oldStartY = this.boxesSelectedInfo.start.y
          this.boxesSelectedInfo.start.y = Math.max(Math.floor(this.boxesSelectedInfo.box.y + dragBoxY), 0);
          this.boxesSelectedInfo.start.y = this.boxesSelectedInfo.start.y + this.boxesSelectedInfo.box.h > this.height ? Math.floor(this.height - this.boxesSelectedInfo.box.h) : this.boxesSelectedInfo.start.y
          dragBoxY = this.boxesSelectedInfo.start.y - oldStartY

          this.boxesSelectedInfo.end.x = this.boxesSelectedInfo.start.x + this.boxesSelectedInfo.box.w
          this.boxesSelectedInfo.end.y = this.boxesSelectedInfo.start.y + this.boxesSelectedInfo.box.h
          ...
          this.metadata.edges.forEach(edge => {
            if (edge.selected) {
              pointArray = edge.d.split(' ').filter(item => item.includes(',')).map(item => item.split(',')).map(item => [Math.floor(Number(item[0])), Math.floor(Number(item[1]))])
              pointArray = pointArray.map(item => {
                return [Math.floor(item[0] + dragBoxX), Math.floor(item[1] + dragBoxY)]
              })
              edge.d = 'M ' + pointArray.slice().map(item => `L ${item.join(',')}`).join(' ').substring(2)
            } else if (nodes.some(node => (edge.source === node.id || edge.target === node.id))) {
              edge.d = edgeData(edge, this.metadata.nodes)
            }
          })
      ...
      } else if (this.boxesSelectedInfo && this.boxesSelectedInfo.status === 1) { // 正在框选
    ...
    svgMouseUp(event) {
      ...
      if (this.selectedLine) {
        if (this.isDragLine) { // 完成节点间连条轨迹的拖曳
      ...
      } else if (this.isDragNode) { // 完成节点拖曳
        this.isDragNode = false;
        if (this.mousedownNode && this.virtualNodeData && (this.mousedownNode.x !== this.virtualNodeData.x || this.mousedownNode.y !== this.virtualNodeData.y)) {
          ...
          this.metadata.edges.forEach(item => {
            if (item.source === this.mousedownNode.id || item.target === this.mousedownNode.id) {
              item.d = edgeData(item, this.metadata.nodes)
            }
          })
          this.notifyDataUpdate()
        }
      } else if (this.isLinking) { // 取消正在连线标示
        this.isLinking = false;
      } else if (this.isDragBox) { // 完成整个框选拖曳
        this.isDragBox = false
        ...
        this.notifyDataUpdate()
      } else if (this.boxesSelectedInfo) { // 框选
        this.unSelectedAll()
        this.clickNode(null)
    ...
    nodemousedown(node) {
      ...
      if (node.selected && this.boxesSelectedInfo && this.boxesSelectedInfo.start && this.boxesSelectedInfo.end) {
        this.isDragBox = true
      } else {
        this.boxesSelectedInfo = null
        this.unSelectedAll();
