      ...
      <el-table :data="tableData" border height="300">
        ...
        <el-table-column label="子流程字段">
          <template slot-scope="scope">
            <el-select filterable clearable v-model="scope.row.key" @change="keyChange(scope.row)">
              <el-option
...
export default {
  ...
  created() {
    let arr = [];
    let jsonData = {};
    if (this.currentMapping.subFieldMaping) {
      jsonData = this.currentMapping.subFieldMaping;
      for (let i in jsonData) {
        arr.push({key: i, value: jsonData[i]})
      }
    }
    this.tableData = arr;
    this.getTemplateDetails(this.currentMapping.processKey).then(res => {
      const { data } = res
  ...
  methods: {
    ...
    ok() {
      ...
      this.$emit('changeMapping', {
        processKey: this.currentMapping.processKey,
        subFieldMaping: JSON.stringify(data) === '{}' ? null: data
      })
    },
    close() {
