  ...
  methods: {
    ...
    checkWorkflow(data) {
      ...
      if (!data.workflow.name) {
        ...
        flag = false;
      } else if (!this.checkUserAttributes(data)) {
        flag = false;
      } else if (!this.checkExternaParams(data)) {
        flag = false;
      }
      return flag;
    },
    ...
    checkTaskNodeApproval(data) {
      ...
      data.nodes.some((node) => {
        if (node.compType === 'task' && !node.workflow.isStartTask && node.workflow.approval && node.workflow.approval.length === 1 && node.workflow.approval[0].type === 'assginee') {
          return false;
        } else if (node.compType === 'task' && !node.workflow.isStartTask && (!node.workflow.approval || node.workflow.approval.length === 0 || node.workflow.approval.every(item => !item.value || item.value.length === 0))) {
          flag = false
    ...
    // 判断参数是否填写正确
    checkUserAttributes(data) {
      let flag = true;
      data.nodes.some(node => {
        if (node.workflow.userAttributes && node.workflow.userAttributes.some(item => !item.field)) {
          message.error(`“${node.label}”：节点的参数模块数据有误` )
          flag = false
          return true
        } else {
          return false
    ...
    // 判断外部系统参数是否填写正确
    checkExternaParams(data) {
      let flag = true;
      data.nodes.some(node => {
        if (node.workflow.externalFlowMaping && (!node.workflow.externaParams || node.workflow.externaParams.length === 0)) {
          message.error(`“${node.label}”：节点的外部系统数据不能为空` )
          flag = false
          return true
        } else if (node.workflow.externalFlowMaping && node.workflow.externaParams.some(item => !item.field)) {
          message.error(`“${node.label}”：节点的外部系统数据有误` )
          flag = false
          return true
        } else {
          return false
