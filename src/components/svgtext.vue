<template>
  <g>
    <text v-if="tempText" style="font-size:12px;opacity:0;user-select:none;">
      <tspan ref="mytspan">{{ tempText }}</tspan>
    </text>
    <template v-if="tspanList.length>0">
      <text v-if="showTextBg" style="font-size:12px;user-select:none;" filter="url(#textBg)">
        <tspan v-for="item in tspanList" :key="item.y" :x="item.x" :y="item.y">{{ item.tspan }}</tspan>
      </text>
      <text v-else style="font-size:12px;user-select:none;">
        <tspan v-for="item in tspanList" :key="item.y" :x="item.x" :y="item.y">{{ item.tspan }}</tspan>
      </text>
    </template>
  </g>
